...
class ESCWN(Check):
	...
	def do(self):
		self.db.Execute('''DROP TABLE IF EXISTS check_ESCWN''')
		self.db.Execute('''CREATE TABLE check_ESCWN (species TEXT,records INTEGER);''')
		...
		for species,count in self.db.Rows('''SELECT species_code,count(*) FROM estimated_subcatch WHERE species_code IN ('ALB') GROUP BY species_code ORDER BY count(*)  DESC;'''): 
			...
			minimum,average = self.dataset.groom_estimated_subcatch_catch[species]			
