...
go
    ...
    def summarise(self,prefix='A'):
        ...
        report += P('''This appendix summarises the extraction, grooming and allocation of landings for the catch and effort data.
        The first section describes the data extract obtained from the Ministry for Primary Industries. There are then sections for each database table
        ...
        Many of the error checks are based on those described in Starr (2011).
        ...
        for table,header,fields,labels in [
            ...
            ('estimated_subcatch','Estimated catch table',['count(*)','sum(catch_weight)/1000'],['Records','Estimated catch (t)']),
            ('fishing_event','Fishing event table',['count(*)','sum(catch_weight)/1000'],['Records','Total estimated catch (t)']),
            ...
            if len(checks)>0:
                ...
                summary = [
                  ['Total','All records including those with errors','-']+list(self.db.Row('''SELECT %s FROM %s;'''%(','.join(fields),table)))
                ]
                for check in checks:
                    result = [0]*len(fields)
                    for row in rows:
        ...
        for species in self.species:
            ...
            for fishstock in self.fishstocks[species]:
                ...
                print>>out, '\t'.join(['fyear','published','qmr','mhr','landings-ungroomed']+checks+['landings-groomed','estimated-areas','allocated-areas','estimated-trips', 'allocated-trips'])
